...
usable on its own. The desktop ([nebelhaus](https://github.com/hausfold/haus))
...
|---|---|---|
| 🏠 [**nebelhaus**](https://github.com/hausfold/haus) | the desktop — nix-darwin modules for tiling, bar, shell, security, palette wiring | anything about *how the system behaves* |
...
| 🪵 [**holt**](https://github.com/hausfold/holt) | the worktree substrate — agent worktrees for any repo and any client, which the desktop takes as a flake input | you want to change how agent worktrees are made, parked or reaped |
...
`~/.config/nix` and is **yours**, never part of the desktop.
...
| **`holt`** | manage [agent worktrees](/guides/claude-agents/) — safe parallel agents, resumable panes | any git repo | the desktop — with `haus.developer.agents.enable` |
...
| **`zscratch`** | [feel-test a zellij edit](/internals/contributing/#feel-testing-a-zellij-edit-zscratch) with no rebuild | the desktop's zellij config | the desktop — with `haus.developer.agents.enable` |
...
*the family repos*. `holt` and `zscratch` are dev tools the desktop puts on your
`PATH` when `haus.developer.agents.enable` is on — `holt` especially is worth
knowing for **anyone** who runs a coding agent. It's the one
that isn't really nebelhaus's at all: [its own
repo](https://github.com/hausfold/holt), which the desktop merely takes as a flake
...
- **[pounce](/guides/pounce/)** installs from Homebrew (`brew install pounce`)
  with zero Nix. It's a general command palette; the desktop just wires it up.
...
The desktop, nebelung and pounce are MIT. Two repos aren't: **holt** is Apache-2.0,
and **perch** is *fair source* — [FSL-1.1-ALv2](https://fsl.software), the
Functional Source License with an Apache-2.0 future licence, so every release
becomes Apache-2.0 two years after it ships. You can read it, build it, and
change it for yourself; you can't sell it as a competing product.
