...
`⇧`+that same key to **throw the focused window there**. Same motion whether
you're heading to a scratch space or to an app, and the two halves of the
thought are one `⇧` apart.
...
| `⇧1`–`⇧4` | Throw the focused window to workspace 1–4 (focus stays where it is) |
| `⇧<app-key>` | Throw the focused window to that app's home workspace |
