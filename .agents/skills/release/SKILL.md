...
```sh
bench release scruff 1.1.1          # or: bench release haus
```
...
All three trusted publishers have now done real work: the `v1.1.0` run published to npm,
PyPI and crates.io over OIDC with no hand step anywhere, eleven jobs green. `scruff`'s own
`docs/releasing.md` carries the table and the bootstrap traps, which still bite a *new*
package — neither npm nor crates.io will show a publisher form for a name that does not
exist yet, so a first release to either has to be published by hand once before CI can.
...
```sh
bench ship                        # or cut with: bench release scruff 1.1.1 --ship
```
