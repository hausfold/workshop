...
| `factory/` — the unattended merge tool | `factory/README.md`, which is the manual — the four verbs, tier 1 and the floor under it, the unknown lines, the budget governor, the watchdog — plus `factory/AGENTS.md` and its two `ai/*/SKILL.md`. No site page: it is a tool the family runs, not something a user installs. ⚠️ **There is no second half in the workshop's `docs/`** — a rule missing from factory's README is missing everywhere. What sits beside it is `haus`'s `docs/night-shift.md`, the layer's four levers (lid, fixer lane, usage feed, trill sources), which is the `haus/modules/*` row's business; keep the tool's own manual out of that one and the haus wiring out of this one |
