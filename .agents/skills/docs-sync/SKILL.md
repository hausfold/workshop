...
| `snug/` — the terminal-presentation runtime | `snug/README.md` and `snug/AGENTS.md`, which ARE the family's presentation standard — the roles, the glyphs, the budget, the record protocol and the two-stream rule. No site page yet: it is a library and a binary the family drives, not something a user installs. ⚠️ There is no second copy to reconcile against: the workshop's `docs/` no longer carries a design half, so a rule that is missing here is missing everywhere |
| `factory/` — the unattended merge tool | `factory/README.md`, which is the manual — the four verbs, tier 1 and the floor under it, the unknown lines, the budget governor, the watchdog — plus `factory/AGENTS.md` and its `ai/SKILL.md`. No site page: it is a tool the family runs, not something a user installs. ⚠️ **There is no second half in the workshop's `docs/`** — a rule missing from factory's README is missing everywhere. What sits beside it is the layer's four levers (lid, fixer lane, usage feed, trill sources), which are split — the operator half on the site as `haus/night-shift.mdx`, the seams under it in `haus`'s `docs/night-shift-internals.md` — and both halves are the `haus/modules/*` row's business; keep the tool's own manual out of that one and the haus wiring out of this one |
| `nebelung/` — palette, ports | `haus/rooms/appearance.mdx` (theming and the palette are one page), `nebelung/README.md` |
| `scruff/` — the worktree substrate + its five SDKs | the **`scruff/` tree**, which is the manual, plus `haus/rooms/ai.mdx` for the user-facing worktree story. What stays in the repo is what a contributor needs: `SPEC.md` (the design of record, and the four frozen contracts), `docs/releasing.md`, `sdk/*/README`, and a `README.md` that is a door. ⚠️ A verb, flag, exit code or `--json` key that moves is a **three-file** change: the code, `ai/SKILL.md`, and the page in that tree. An SDK surface change is also a release question — see `/release` |
| `haus/modules/*` | the matching **room** under `haus/rooms/`, and `haus/desktops/*` when a desktop's values move. haus's own `docs/` is contributor material and generated data, never a manual: `model.md` and `macos-settings.md` are pinned by name from a dozen source files each, `focus.md` is the design record for a room built with no public API, `night-shift-internals.md` is the contract the unattended merge tool reads this machine through (the usage TSV's columns most of all — haus writes nine, a budget gate reads four, and nothing checks the order), with the half a person SETS and RUNS on the site as `haus/night-shift.mdx`, and 🚨 `docs/site-data/` is what the row below regenerates the options reference *from* — never prune it. `README.md` is a door |
...
| `homebrew-tap`, release CI | `haus/install.mdx`, `haus/keeping-it-current.mdx`, and `scruff/install.mdx` — the tap carries scruff's formula as well as pounce's and perch's |
| `.github/workflows/**` in **any** repo | the workshop's [`docs/ci.md`](../../../docs/ci.md). Its gate table names every repo's workflows and the runners they sit on, and its five rules are stated as binding on all of them — so a job added, split, renamed or moved makes a doc wrong in a repo it does not live in, which is the one shape nothing in that repo's own CI can notice. A *release* workflow is out of that doc's scope by design, except rule 4 |
| `org-profile` (the `hausfold/.github` repo) | `profile/README.md`, the org front page and the first thing anyone sees, and the whole of it — the banner and its `profile/assets/` came off, so there is no second file there. Reconcile its repo list against `content/docs/meta.json` and the `#made` list in hausfold.co's `src/app/page.tsx`; there is no `/docs` index page, only the five trees |
...
| 🔒 anything that dates a claim in the **private** `ops` repo | `ops/PRESENCE.md` (the register: a repo renamed or created, a package published to npm/PyPI/crates, a namespace won or lost, a domain or handle moved), `ops/MARKETING.md` (a pipeline that got built, a tool that got installed, a blocker that cleared), `ops/scoreboard/` (a **new public repo** has to be added to `collect.sh`'s `REPOS` by hand — one that isn't listed reports as nothing, which looks exactly like a repo nobody visits), `ops/LANDSCAPE.md` and `ops/TESTERS.md` (hand-maintained; correct them only against something that landed). **One-way: nothing from `ops` goes into any other repo's PR.** ⚠️ `ops/log/` is the exception to the whole step: those files are closed records, and that folder's own second rule is that a record is **not** edited to keep it current — when the thing one describes changes, the change belongs where the thing lives now and the record is left alone |
