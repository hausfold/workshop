...
|---|---|
| `pounce/` — the launcher | the **`pounce/` tree** (`config.mdx` carries every `config.json` key), plus `haus/rooms/launcher.mdx` for the *haus wiring only*. A room page documents the room; the app is documented in the app's tree. ⚠️ **pounce has no `docs/` any more** — the tree IS the manual (2026-08-29), so a fact that used to go in `docs/reference.md` goes in the tree, and the repo's `README.md` is a door: what it is, one install line, links. Don't grow it back |
| `perch/` — the notch file shelf | the **`perch/` tree** — which is the manual; `docs/reference.md` was retired into it on 2026-08-29 and what is left in `docs/` (`cli.md`, `feel-testing.md`, `app-store.md`) is for someone working on perch. Plus `haus/rooms/shelf.mdx` for the wiring, and `haus/keeping-it-current.mdx` for anything install-shaped (perch ships as a cask). ⚠️ **The tree is the only page about perch now**: its landing sheet at `/perch` was retired on 2026-08-26 (it 301s to `/docs/perch/`), so there is no second surface to keep in step — but `/perch/privacy` survives as a route, because the App Store listing points at it, and a behavior claim in that policy is still a two-file change |
| `trill/` — the notification compositor | `trill/docs/*` — the one app of the four whose **manual is still in-repo**, because the site's `trill/index.mdx` is a single incubator page held there deliberately (hausfold.co's `AGENTS.md`) until the tab's positioning is decided. Plus `trill/ARCHITECTURE.md`, and `trill/README.md`, which is a door onto `docs/` rather than a copy of it |
