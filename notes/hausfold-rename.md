...
| on-disk `~/code/workshop/trill` | the archived clone | ✅ `~/code/workshop/messages`, remote repointed, `.gitignore` and `bench`'s `FAMILY` entry following (§3.1's rule: each checkout named for its repo). The dir is what the eject's final `mv` needs. ⚠️ **workshop#283 then reaped the workshop's side of it entirely**: the `FAMILY` entry and the `.gitignore` line are both gone, so `bench clone` no longer fetches it and `bench status` no longer reports it. ⚠️ **But the checkout is still on disk and is now UNTRACKED** — `~/code/workshop/messages` exists (remote `nebelhaus/messages`, HEAD `88a4ddd`), and dropping the `.gitignore` line without deleting the dir leaves the workshop's `git status` showing `?? messages/` forever. A `git add -A` up there would try to add a nested repo. Decide it deliberately: `rm -rf` the checkout (the repo is archived on GitHub and stays downloadable), or put the ignore line back. Don't let it sit as permanent noise. |
