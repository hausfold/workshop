...
| **3. Desktop seam** | blocked by 2 | Add exactly-one-desktop selection, source attribution, closed-schema validation, desktop-safety enforcement and host-wins priority. Keep the compatibility builder selecting nebelhaus implicitly. Do not design remote acquisition here. | A named haus flake check with positive fixtures for one desktop, host override and source diagnostics; negative fixtures for two desktops, module functions, `imports`, `_module`, extra top-level keys, `system.activationScripts`, unknown options and every class of host-only leaf. | One desktop is selected; a plain host assignment overrides it; a second is rejected clearly; source filenames survive diagnostics; only the closed `{ haus = { … }; }` value reaches option evaluation; desktop data cannot set identity, secrets, account coordinates, signing identities or hardware identifiers. |
| **4. Carve out nebelhaus** | blocked by 3 | In one atomic change, neutralize generic room defaults, add the real nebelhaus desktop and add the built-in Blank desktop. Keep `mkNebelhaus` and old option addresses as compatibility surfaces. | Commit before/after **allowlisted behavioral projections** for the example and real consumer: desktop-safe option values plus non-sensitive package/service identifiers, with host-only and secret-bearing paths excluded by construction. Add a Blank fixture; run `nix flake check` and `bench try`. Record projection commands and exclusions in the PR. Full consumer evaluations may run, but their raw output remains ephemeral and is never committed or attached. | Existing nebelhaus projections are identical; Blank enables no optional rooms; the projection contains no identity, secret, account, signing or hardware values; there is no commit on `main` where existing installs silently lose a room. |
