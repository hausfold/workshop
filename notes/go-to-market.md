...
| 6 | Where does `PRESENCE.md` live once the site repo is public? | **Decided 2026-08-08: a new private `hausfold/ops` repo.** Keeps the file in git with its history and gives the rest of the ops surface a home — where credentials live (not the credentials), account facts, the annual re-check. ⚠️ The wrong answer, recorded so it isn't re-derived: **not** `notes/` in this repo — **the workshop is public**, so that move publishes the gap list rather than protecting it. |
