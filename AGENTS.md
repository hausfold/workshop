...
| **what a push costs before you know it's green** — a trigger that fires twice, a tool asked of apt that the runner image already has, a Nix store rebuilt from nothing every run, a third-party action pinned to `@main` | [`docs/ci.md`](docs/ci.md), binding on every repo: four rules, where the Nix store cache is on and why it is not everywhere, and the two things — a third-party runner fleet, a hosted binary cache — we've looked at and said no to. Every repo here is public, so minutes are free and the only cost is the wall clock between pushing and knowing. A release workflow is out of scope by design |
