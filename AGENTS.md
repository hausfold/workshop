...
| holt — the worktree-lifecycle substrate (`wt` extracted as its own Go product) | `./incubator/holt` — **incubating**: a Go repo-to-be, design in [`SPEC.md`](./incubator/holt/SPEC.md), all 77 of `wt`'s acceptance tests passing. **`wt` is FROZEN** (2026-08-03): bugfixes only, no new features — every behaviour change goes here instead. `wt` in `./nebelhaus/modules/den/wt.sh` stays the live implementation until cutover; a bugfix that lands there must be ported here in the same session, or the two diverge. |
