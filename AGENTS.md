...
| **a write-up that turned out to be wrong** — a stale claim, a check that passes while what it protects rots | [`docs/drift.md`](docs/drift.md): thirty-four numbered shapes and what catches each. **Row numbering is frozen** — cite by number. Append a shape, or park it under *Seen once, not yet a row* |
...
| **what a push costs before you know it's green** — a trigger that fires twice, a tool asked of apt that the runner image already has, a Nix store rebuilt from nothing every run, a third-party action pinned to `@main`, a job per suite that pays its own setup over again without moving the gate | [`docs/ci.md`](docs/ci.md), binding on every repo: five rules, where the Nix store cache is on and why it is not everywhere, and the two things — a third-party runner fleet, a hosted binary cache — we've looked at and said no to. Every repo here is public, so minutes are free and the only cost is the wall clock between pushing and knowing. A release workflow is out of scope by design |
...
| scruff's or pounce's Homebrew formula, perch's cask | `./homebrew-tap` — **CI-owned**; hand-edit only to bootstrap a new formula/cask. scruff's is the one entry that COMPILES (a Go build from the tag tarball, because a formula-installed CLI is never quarantined and has nothing to sign), so its CI-owned pair is `url` + `sha256` and it carries no `version` line — Homebrew scans that from the URL and `brew audit` rejects the second copy. It is also the only entry the tap's gate covers |
...
| a standard that binds every repo | this repo's [`docs/`](./docs/): `agent-surface.md`, `bug-reports.md`, `ci.md`, `design.md`, `drift.md`, `agent-vm.md` |
