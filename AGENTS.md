...
- `./bench status` — leads with **what this machine is actually running**
  ...
  `worktree-*` branch — a worktree holt's registry doesn't know (a hand-run
  `git worktree add`) prints **◇**, because `holt reap` will never sweep it.
  It also flags an **OFF-MAIN** edge — a lock pinned at a
