...
| **a write-up that turned out to be wrong** — a stale README claim, a comment that outlived its code, a check that passes while the thing it protects rots | [`notes/drift.md`](notes/drift.md), which lives here because it binds every repo: twenty-four numbered shapes and, beside each, the only thing that catches it. **Row numbering is frozen** — commit messages and source comments in `haus` cite rows by number. Append a shape when you find one the table can't already name; the passes that find them are at the bottom of that file |
