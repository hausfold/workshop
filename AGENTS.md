...
| the hausfold.co site, or the hausfold name register | `./hausfold` — **its own repo**, [hausfold/website](https://github.com/hausfold/website), split out of here 2026-08-06 with its history. Until 2026-08-08 it was *the only one outside the `nebelhaus` org*; the [hausfold rename](notes/hausfold-rename.md) inverts that — every repo migrates in, and this one stops being the exception. One static `index.html` on a Cloudflare Worker, deployed by CI on push to its `main`. `bench clone` fetches it; it is **not** a flake input and not part of `FAMILY`. |
