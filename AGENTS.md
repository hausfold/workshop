...
| **the night shift** — the merge lease, what counts as tier 1, the runner that drives it | the TOOL is [hausfold/factory](https://github.com/hausfold/factory), whose README is the manual and which haus puts on `PATH` (`haus.ai.enable`) along with its one skill, `/factory`. **A flake input that is not a `FAMILY` repo** — trill and snug's exact footing, so it is in `OVERRIDABLE`, `EDGES` (`haus → factory`), `DOCS_REPOS`, `bench clone` and `bench pull`, while `bench ship` never pushes it — it lands through its own PRs. It is also the only repo that HOLDS an edge bench does not walk (`factory → snug`, for the presentation runtime), which is why `bench status`'s row for it names factory's own PR instead of a verb. Its reports draw through snug: `lib/ui.sh` sources `share/ui.sh` off `FACTORY_UI_SH`, set by its Nix wrapper, and degrades to plain marked text without it, so the clone-and-symlink install keeps working. ⚠️ **Nothing about the night shift lives in this repo.** The manual is factory's own README; the four levers a shift reaches for on a haus machine — the lid, the fixer lane, the usage feed it meters against, and the trill sources a night draws under — are named in two halves and nowhere else: what a person SETS and RUNS is hausfold.co's `docs/haus/night-shift`, and the seam each of those four can break at silently is `./haus`'s `docs/night-shift-internals.md`. The policy itself is machine-local because it is authority, so `factory config print` is the only current statement of it and a copy in a watched repo would be a file a PR could edit to widen the filter judging it. A live lease (`factory lease status`) is the user's standing go-ahead for **tier-1** merges exactly as the policy decides them — the shift merges those without a fresh prompt; everything else still waits at "PR open" |
...
- **Land your work through a PR — never a direct push or a local `git merge`
  ...
  see the night-shift row in the routing table.
