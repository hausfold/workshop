...
| **how the brand looks off the terminal** — a logo, a lockup, a product's hue, a banner, an OG card, a README hero | [`docs/design.md`](docs/design.md), which lives here because it binds every repo: the shared ears mark, the tile, per-product marks and hues, typography, the lockup geometry, and what is deliberately not defined yet. Colours resolve against **nebelung**'s tokens (that repo stays the source of truth for the values); how a **CLI** draws stays snug's standard (the row above), and how **hausfold.co** implements the system stays that repo's own `AGENTS.md`. The master SVGs live in the *Logo system* design project — the doc carries their geometry so a repo can rebuild an asset without it |
