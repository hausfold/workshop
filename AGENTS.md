...
| **the install one-liner** — the URL, which desktop it resolves, the ref pinning | `./hausfold.co`'s `worker.js`, since 2026-08-14. It is `curl -fsSL https://hausfold.co/nebelhaus.sh \| bash` now. `./web` still answers the old `nebelhaus.com/init.sh` with the same script, and keeps doing so until the 301s land ([§5.2](notes/hausfold-rename.md#52--the-move--and-the-salvage-list)) — so a change to *how installing works* belongs in both until then, and a change to the *script* belongs in `./haus`'s `bootstrap.sh` and neither |
| the rest of nebelhaus.com — the old landing page and the Astro/Starlight docs | `./web`, and it is on its way out: every page here has a rebuilt equivalent on hausfold.co, and what remains is turning this site into the 301 map |
