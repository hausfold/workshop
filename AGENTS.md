...
| the archived **Messages client** (iMessage/SMS/RCS) that used to be called trill | `nebelhaus/trill` — **archived 2026-08-04, read-only, and it stays in the `nebelhaus` org** rather than transferring with everything else. It is also **renamed in place** (name pending; unarchive → rename → re-archive), which is what frees the `trill` name for the row above. Its last release stays downloadable through GitHub's rename redirect, but its deprecated `Casks/trill.rb` is scheduled for retirement — two casks cannot hold one token. All of it: [`notes/hausfold-rename.md`](notes/hausfold-rename.md) §3.4. |
