...
/// second one is a claim about macOS, which flick must not make on a button
...
final class WalkthroughTests: XCTestCase {
    ...
    /// The one that matters: flick may only say "macOS has stopped drawing
    ...
    func testOneUserAdvancedAppMeansMacOSNeverConfirmedIt() {
        ...
        XCTAssertFalse(
            ...
            "an app flick never verified must not be reported as verified"
        )
    }
