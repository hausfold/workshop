{
  description = "Flick — a quiet, scriptable notification compositor for macOS";

  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixpkgs-unstable";

    # Injection point for feel-testing a flick SOURCE branch through `bench try`.
    # macOS 26 blocks a from-source Nix build (see nix/package.nix), so bench
    # builds Flick.app from the branch in your login session and overrides this
  ...
  outputs =
    ...
    in
    {
      # Consume flick from anywhere: `overlays.default` puts `flick` into pkgs.
      # The rice adds this overlay and installs pkgs.flick in place of the cask.
      overlays.default = final: prev: {
        flick = final.callPackage ./nix/package.nix {
