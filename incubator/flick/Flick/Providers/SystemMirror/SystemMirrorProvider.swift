...
/// **System Mirror — experimental.** The trill move applied to notifications:
...
/// unlike trill's `chat.db` — is an undocumented implementation detail that
...
struct SystemMirrorProvider: NotificationProvider {
    ...
    func probe() async -> ProviderHealth {
        ...
        guard sqlite3_open_v2(storePath, &db, SQLITE_OPEN_READONLY, nil) == SQLITE_OK, let db else {
            sqlite3_close(db)
            return .unavailable(reason: "usernoted store unreadable (grant Full Disk Access to Flick)")
        }
