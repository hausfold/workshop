...
**deliberately empty** (no `Flick.app`). When it's empty, `nix/package.nix`
fetches the CI-built release ZIP as normal.

`bench try` / `bench try-batch` use it to feel-test a flick **source** branch
...
`../package.nix`), bench builds `Flick.app` from the branch **in your login
session** (where `xcodebuild` works), signs it with a stable identity under the
`com.nebelhaus.flick.dev` bundle id, and points this input at that build:

```
--override-input nebelhaus/flick/prebuilt path:/path/to/built-app-dir
```
