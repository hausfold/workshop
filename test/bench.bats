...
# haus (haus/modules/core) — bench no longer parses hook payloads.)
...
#
# The measuring moved to scruff (`scruff overlap`, its own suite); what is left
# here is the forwarding, and the sentence a too-old scruff gets.

mkscruff() { # mkscruff <body> — a scruff on PATH whose script is <body>
  mkdir -p "$TMP/fakebin"
  printf '#!/bin/sh\n%s\n' "$1" >"$TMP/fakebin/scruff"
  chmod +x "$TMP/fakebin/scruff"
  PATH="$TMP/fakebin:$PATH"
}

@test "cmd_overlap hands everything to scruff overlap, exit code included" {
  mkscruff 'if [ "$1 $2" = "overlap --help" ]; then printf "  scruff overlap  the verb\n"; exit 0; fi; printf "scruff %s\n" "$*"; exit 4'
  run cmd_overlap --brief --path AGENTS.md
  [ "$status" -eq 4 ]
  [ "$output" = "scruff overlap --brief --path AGENTS.md" ]
}

@test "cmd_overlap says where the verb went when the scruff on PATH predates it" {
  # A 1.4.0 scruff reads `overlap` as a lane name and answers "no lane named
  # overlap", which is true and useless. Its --help falls back to the whole
  # usage, which has no `scruff overlap` block — that is the tell.
  mkscruff 'printf "scruff — the worktree-lifecycle substrate\n"; exit 0'
  run cmd_overlap
  [ "$status" -eq 1 ]
  [[ "$output" == *"scruff overlap"* ]]
  [[ "$output" == *"haus update"* ]]
}
