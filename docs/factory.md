...
| `script/factory-tier` | is a PR **tier 1** — mergeable by code alone? Docs-only (`*.md`, `docs/`, `content/`), by the org owner, from a `worktree-*` branch onto `main`, green, conflict-free, ≤2000 changed lines. Agent-steering files (`AGENTS.md`, `CLAUDE.md`, any `SKILL.md`, `.github/`, `.claude/`, `.agents/`) are never tier 1 however docs-shaped: a policy change always meets a person. The filter IS the definition — widening it is a reviewed edit to that script plus this file |
| `script/factory-shift` | one deterministic pass: print the budget line, run every open org PR through the tier check, merge tier 1 under a live lease (`gh pr merge --squash`), `bench pull` + `bench ship` if anything landed, flag a red latest run on any repo's `main` as `CI-RED` (+ a trill fault card, `--source factory`). Appends every line to `~/.cache/hausfold-factory/shift-YYYYMMDD.log` — the morning report is that file. `--dry-run` senses and merges nothing |
