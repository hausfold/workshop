...
This project moves fast. Every desktop tweak, palette nudge, or bar rework dates a
...
| 2 | **Desktop hero** | `haus/assets/hero.png` | the launch posts — r/unixporn, Show HN, the one-pager, the reel's poster; **the haus README carries no capture today** | For a desktop, the screenshot *is* the pitch — one clean tiled desktop + bar. ⚠️ **A frame that carries a stale wordmark or org name, a username, an uptime or a battery percentage is disqualified** — a brand that has moved dates the shot, and the rest names the shooter. The org's *current* name and mark are not a strike: the frame below sits a browser on `github.com/hausfold` on purpose.  ⚠️ **A Ghostty window IS a pane**, with no tab bar or keybind hint strip of its own, so a bare shell photographs as an empty rectangle and the window needs a TUI in it. The frame: Zen on `github.com/hausfold` (grey and pink via the `github` entry in `haus.zen.userStyles`) beside Ghostty running lazygit over an unstaged `haus.focus.scenes` tree, Pounce open on its **empty palette** so the action tiles are the row a stranger reads first, both bars on. ⚠️ **The palette is empty on purpose** — a typed query highlights one row and hides the tiles, which are the part that says what the launcher is for. ⚠️ **Pounce draws the top clipboard entry in the palette**, so whatever was last copied is in shot and has to be chosen. Two costs to respect — **selecting a commit** puts `Author: … <email>` in lazygit's main panel, and an email address is disqualifying; the Commits panel's own rows carry initials only and are fine in shot, as is Files. And a ~34-column pane wraps past ~40 characters, so config being shown off must be written short. ⚠️ **Do not reach for `+`**: half-screen mode widens the file tree, which squeezes the diff panel to ~22 columns and wraps every line of nix in the panel the frame exists for. Normal mode gives the diff half the width and it reads clean. ⚠️ **Two things in the frame are the shooter's host, not the desktop** — `haus.zen.userStyles` defaults to `[ ]` and `haus.bar.bottom.enable` to `false`, and `desktops/hacker.nix` sets neither, so a fresh install has an unthemed browser and one bar. Launch-post copy that leads with this image has to say so. `hausfold/ops`'s `todo/launch-phase-1.md` carries the full measurement. Nothing in the family renders it; the consumers are r/unixporn, the Show HN, the creator one-pager and the reel's poster frame. |
...
- A clean morning clock (**9:41** by convention), a fog-appropriate weather
  city, neutral clipboard/tabs, fresh git state. The point is a time that says
  nothing about the shooter; 2:07 fails that and 9:44 does not.
...
this table, and it is the stage that caught four of them in a single take.
