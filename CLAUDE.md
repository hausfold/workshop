...
| Skills (`/ship`, `/docs-sync`, `/release`, `/earshot`) | `.claude/skills/<name>/SKILL.md` | Symlinks into `.agents/skills/` — the shared bodies every client uses. Edit the target, never the link. `/nightshift` is not here: factory ships it, and haus installs it into every client. |
