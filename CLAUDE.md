...
| Worktree hooks | `~/.claude/settings.json` (yours, not the repo's) → `holt hook create` / `holt hook remove` | **The rice declares these** (nebelhaus#201) and re-asserts them every rebuild, so they self-heal when Claude rewrites the file. Not hand-edited — change them in `modules/terminal`. The four agent-state hooks in the same file stay yours. |
